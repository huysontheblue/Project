...
select dbo.KF_Check_SV_In_Khoa('1851', 'CNTT');
select dbo.KF_Check_SV_In_Khoa('1857', 'VL');
select dbo.KF_Check_SV_In_Khoa('1858', 'CNTT');
go
...
select dbo.KF_Last_Score_Of_Student('1857', 'MH1')
select dbo.KF_Last_Score_Of_Student('1851', 'MH2')
select dbo.KF_Last_Score_Of_Student('1854', 'MH4')
go
...
select dbo.KF_Last_AVG_Score_Of_Student('1851')
go
...
select * from dbo.KF_Last_List_Score_Of_Student('1854', 'MH4')
go
-- 5. Nhập vào 1 sinh viên, trả về danh sách các môn học mà sinh viên này phải học. 

create function dbo.KF_List_MH
(
	@masv varchar(10)
)
returns table
...
select * from dbo.KF_List_MH('1858')
go